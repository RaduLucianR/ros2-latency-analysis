from launch import LaunchDescription
from launch.actions import TimerAction, Shutdown, DeclareLaunchArgument
from launch_ros.actions import Node
from tracetools_launch.action import Trace
from launch.substitutions import LaunchConfiguration
from ament_index_python.packages import get_package_share_directory
import os
import time

def generate_launch_description():
    executors = 'two_nodes_m1'
    
    config_cores = os.path.join(
      get_package_share_directory('simple-chain'),
      'config',
      'two_nodes_1.yaml'
      )
    
    config_payload = os.path.join(
      get_package_share_directory('simple-chain'),
      'config',
      'two_nodes_payload.yaml'
      )
    
    return LaunchDescription([
        DeclareLaunchArgument(
            'trfolder',  # Name of the argument
            default_value=time.strftime(f"e2e_{executors}_%Y%m%d-%H%M%S"),  # Default value
            description='Trace folder name'
        ),

        Trace(
            session_name=LaunchConfiguration('trfolder'),
            events_kernel=[],
            events_ust=['ros2*']
        ),

        Node(
            package='simple-chain',
            executable=executors,
            arguments=[config_cores],
            parameters=[config_payload]
        ),

        TimerAction(
            period=5.0,  # time in seconds after which the shutdown will be triggered
            actions=[
                Shutdown(reason='Time limit reached')
            ]
        )
    ])
