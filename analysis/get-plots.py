...
def main(trace_folder_path, trace_path_name, destination_path):
    ...
    cl_node.render(filename = f'cl-node-{trace_folder_name}', directory = destination_path, format='png')
    cl_end_to_end.render(filename = f'cl-e2e-{trace_folder_name}', directory = destination_path, format='png')

    histogram = Plot.create_response_time_histogram_plot(path)
    histogram_fig = histogram.figure()
    export_png(histogram_fig, filename = f"{destination_path}/histo-{trace_folder_name}.png")

    stacked_bar = Plot.create_response_time_stacked_bar_plot(path)   
    stacked_bar_fig = stacked_bar.figure()
    export_png(stacked_bar_fig, filename = f"{destination_path}/stacked-{trace_folder_name}.png")
    ...
    export_png(message_flow_node_fig, filename = f"{destination_path}/mf-node-{trace_folder_name}.png")
    ...
    export_png(message_flow_e2e_fig, filename = f"{destination_path}/mf-raw-{trace_folder_name}.png")

    cb_latency = Plot.create_latency_timeseries_plot(app.callbacks)
    cb_latency_fig = cb_latency.figure()
    export_png(cb_latency_fig, filename = f"{destination_path}/cb-lat-{trace_folder_name}.png")
    
    records = path.to_records()
    response = ResponseTime(records)
    response_time_records = response.to_all_records()
    response_df = response_time_records.to_dataframe()
    response_df.to_csv(f"{destination_path}/resp-time-{trace_folder_name}.csv")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Get and save plots and data for a trace folder.")
    parser.add_argument("trace_folder_path", type=str, help="Path to the trace folder. \
                        Assuming its architecture yaml file is in the parent directory of the trace folder.")
    ...
    main(args.trace_folder_path, args.trace_path_name, args.destination_path)
